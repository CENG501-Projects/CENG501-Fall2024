"""
MonoATT: Adaptive Token Transformer for Monocular 3D Object Detection
"""
import torch
...
from lib.losses.focal_loss import sigmoid_focal_loss, focal_loss
from .dn_components import prepare_for_dn, dn_post_process, compute_dn_loss
from .cluster_center_estimation import build_cce
from .adaptive_token_clustering import build_atc
...
class MonoATT(nn.Module):
    """ This is the MonoATT module that performs monocualr 3D object detection """
    def __init__(self, backbone, cce, atc, depthaware_transformer, depth_predictor, num_classes, num_queries, num_feature_levels,
    ...
    def forward(self, images, calibs, targets, img_sizes, dn_args=None):
        ...
        #Use mid level features for cluster center estimation.
        cce_scores, cluster_centers, tokens, token_positions = self.cce(features[1], calibs)
        cluster_assingmnents, merged_features = self.atc(tokens, cluster_centers, token_positions)
        #Merged features [16, 1920]: Cluster number for each token.
        #Add merged features into feature list.
        features.append(merged_features)

        srcs = []
        masks = []
        for l, feat in enumerate(features):
            if l == len(features) - 1: #Handle speatical features. (Last one)
                feat = feat.float()
                batch_size, _, _ = feat.shape
                #TODO: Those should be calculated based on the transformet output. For now hardcoded.
                num_channels = 512  
                dim1 = 10
                dim2 = 5
                feat = feat.reshape(batch_size, num_channels, dim1, dim2)
                srcs_special = self.input_proj[0](feat)
                m = torch.zeros(srcs_special.shape[0], srcs_special.shape[2], srcs_special.shape[3]).to(torch.bool).to(srcs_special.device)
                mask_special = F.interpolate(m[None].float(), size=srcs_special.shape[-2:]).to(torch.bool)[0]
                pos_special = self.backbone[1](NestedTensor(srcs_special, mask_special)).to(srcs_special.dtype)
                srcs.append(srcs_special)
                masks.append(mask_special)
                pos.append(pos_special)
            else:
        ...
        out['cluster_center_scores'] = cce_scores.to(outputs_class.device)
...
class SetCriterion(nn.Module):
    """ This class computes the loss for MonoATT.
    ...
    def loss_cce(self, outputs, targets, indices, num_boxes):
        # Extract predicted heatmap logits from outputs

        predicted_heatmap = outputs['cluster_center_scores']  # Shape: [B, H, W]
        B, H, W = predicted_heatmap.shape

        # Initialize ground truth heatmaps
        gt_heatmaps = []
        for target in targets:
            # Create heatmap ground truth
            gt_heatmap = torch.zeros(H, W, device=predicted_heatmap.device)
            # Extract 2D boxes
            boxes = target['boxes']  # [N, 4] with format [x_center, y_center, width, height]
            centers = boxes[:, :2]  # Take (x_center, y_center)
            # Scale centers to feature map resolution
            centers[:, 0] *= W  # Scale x-coordinates
            centers[:, 1] *= H  # Scale y-coordinates
            for center in centers:
                x, y = int(center[0]), int(center[1])
                if 0 <= x < W and 0 <= y < H:
                    gt_heatmap[y, x] = 1  # Mark the center on the heatmap
            gt_heatmaps.append(gt_heatmap)
        
        # Stack heatmaps to form a batch
        gt_heatmaps = torch.stack(gt_heatmaps)  # Shape: [B, H, W]

        # Compute sigmoid focal loss
        loss = sigmoid_focal_loss(
            inputs=predicted_heatmap,
            targets=gt_heatmaps,
            num_boxes=num_boxes,  # Normalization factor
            alpha=self.focal_alpha,           # Focal loss alpha parameter
            gamma=2.0             # Focal loss gamma parameter
        )

        losses = {}
        losses['loss_cce'] = loss
        return losses
        ...
        if 'aux_outputs' in outputs:
            for i, aux_outputs in enumerate(outputs['aux_outputs']):
                ...
                for loss in self.losses:
                    ...
                    kwargs = {}
                    if loss == 'cce':
                        #There is no aux loss for cce_loss
                        continue
                    if loss == 'labels':
...
def build(cfg):
    ...
    #cluster_center_estimation
    cce = build_cce(cfg)

    #adaptive_token_clustering
    atc = build_atc(cfg)
    ...
    weight_dict['loss_cce'] = cfg['cluster_center_loss_coef']
    ...
    losses = ['labels', 'boxes', 'cardinality', 'depths', 'dims', 'angles', 'center', 'depth_map', 'cce']
